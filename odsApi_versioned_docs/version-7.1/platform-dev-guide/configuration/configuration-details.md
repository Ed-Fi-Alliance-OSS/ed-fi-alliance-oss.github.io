...
| EdFi.Ods.SwaggerUI | appsettings.json | Tenants | Example:<br/><br/>```<br/> "Tenants": [<br/>    {<br/>      "Tenant": "Tenant1"<br/>    }<br/>  ],<br/>``` | Optionally provides the name of a tenant. This setting is required if using [Multi Tenant Configuration](./single-and-multi-tenant-configuration.md#multi-tenant-configuration). A valid tenant name is required for SwaggerUI to retrieve the API metadata. |
...
| EdFi.Ods.WebApi | appsettings.json | ApiSettings:Caching:Redis:Configuration | Example: [see this document](https://stackexchange.github.io/StackExchange.Redis/Configuration.html) | When `ExternalCacheProvider=="Redis"` , provide the connection string information here. Default: an empty string |
| EdFi.Ods.WebApi | appsettings.json | ApiSettings:Caching:Security:AbsoluteExpirationMinutes | Example: 10 | The amount of time the security metadata from `EdFi_Security` database is cached. E.g., if it is set to 10 mins, the claim set changes will reflect in the API at least after 10 mins without needing to recycle API process. |
...
![.NET CLI Tool](https://edfi.atlassian.net/wiki/download/thumbnails/25493635/image2021-3-19_12-8-29.png?version=1&modificationDate=1699456103973&cacheVersion=1&api=v2&width=1097&height=80)
...
![Manage User Secrets](https://edfi.atlassian.net/wiki/download/thumbnails/25493635/image2021-3-19_12-6-13.png?version=1&modificationDate=1699456103957&cacheVersion=1&api=v2&width=773&height=247)
...
![Secret.json example](https://edfi.atlassian.net/wiki/download/thumbnails/25493635/image2021-3-12_16-47-40.png?version=1&modificationDate=1699456103947&cacheVersion=1&api=v2&width=710&height=180)

See [Safe storage of app secrets in development in ASP.NET
Core](https://docs.microsoft.com/en-us/aspnet/core/security/app-secrets?view=aspnetcore-3.1&tabs=windows)
for more details.
