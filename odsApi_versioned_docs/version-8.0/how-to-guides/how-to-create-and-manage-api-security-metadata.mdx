...
**Upload** the modified document to replace the active claims hierarchy:
...
`claimsHierarchy` array. It atomically replaces the current configuration and
re-seeds the `ResourceClaim` catalog with any new claim names.
