...
organized into a schema per project (for example, the `ed-fi` project maps to
the `edfi` schema, giving tables such as `edfi.School` and `edfi.Student`) in
PostgreSQL or SQL Server. Unlike the ODS/API's deeply normalized,
inheritance-based schema, v8 uses a flat tables-per-resource model with a
...
The Configuration Service manages data stores (tenant database connections,
encrypted at rest), and tenant context is extracted directly from the URL route.
Multiple tenants can share a single deployment, with complete data isolation
...
Service. See
[Multi-Tenancy](../platform-dev-guide/configuration/single-and-multi-tenant-configuration.md)
