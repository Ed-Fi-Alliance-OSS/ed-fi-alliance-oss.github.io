...
Throughout this documentation, we generally use "HTTP" to describe specific
uses of that protocol. As noted elsewhere, the traffic between systems
containing production data is always over HTTPS.
...
created, the SIS system will be able to perform other operations on that student
record. The same is applicable to staff and contacts.
...
Resources](../platform-dev-guide/security/api-claim-sets-resources.md) page.

A namespace-based authorization strategy is used for resources such as
assessment metadata, where access is governed by the namespace prefixes assigned
to the caller rather than by a relationship to an education organization.

## API Client Profiles

API Profiles enable an Ed-Fi API platform host to constrain the data exposed
from specific resources to meet the needs of individual use cases. When an API
client is assigned a profile, API responses for resources covered by the profile
are served on a profile-specific media type. To use a profile, callers must add
media-type information to their requests.
...
```text
application/vnd.ed-fi.{resource}.{profile-name}.{readable|writable}+json
```

Where:

- `{resource}` — The API resource name, lowercased (e.g., `student`, `school`)
- `{profile-name}` — The name of the profile as declared in the [profile
  definition](../platform-dev-guide/security/api-profiles.md#profile-definition)
  (e.g., `nutrition`, `assessment`)
- `{readable|writable}` — Operation type: `readable` for read operations,
...
This indicates to the server that the caller will accept the profile-based
...
This indicates to the server that the caller expects their update to apply to
the constrained surface area of the resource as defined by the profile.
...
API clients that have been assigned only one profile covering the accessed
resource can omit the media-type headers or use standard `application/json`,
and the Ed-Fi API will auto-apply the assigned profile. API clients assigned
with more than one profile covering the accessed resource MUST include the
media-type headers for one of the applicable profiles. Failing to provide a
valid profile header will result in an error response.
...
Combining the `/oauth/token_info` endpoint with the [Dependency
Order](./resource-dependency-order/readme.mdx) endpoint could make integrating
with the Ed-Fi API easier.

:::

The process is relatively straightforward. POST to the
`/{pathBase}/oauth/token_info` endpoint with an HTTP Authorization header as
"Bearer XYZ" where "XYZ" is the access token. The same access token must also
be sent in the request body in the `token` parameter. Note that you need an
active bearer token for this call, which you can obtain from the Configuration
Service `/connect/token` endpoint. See
[Authentication](./authentication.md) for a refresher on how to acquire
a token.
...
The snippet below shows a sample response. For a description of each field,
refer to the [OpenAPI
specification](https://github.com/Ed-Fi-Alliance-OSS/Ed-Fi-API-Specifications/blob/main/api-specifications/oauth-token-introspection/1.0-draft/oauth-token-introspection-api-1.0.yaml).
