...
Core](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/configuration/?view=aspnetcore-10.0)
...
```
  ...
  <tr style={{verticalAlign: 'top'}}>
    <td>:new: ShowDomains</td>
    <td>true</td>
    <td>Displays Ed-Fi resource domain badges in Swagger UI (default: true).</td>
  </tr>
  <tr style={{verticalAlign: 'top'}}>
    <td>:new: StartupClassName</td>
    <td>CustomStartup</td>
    <td>Specifies the name of the Startup class the application should use. This allows overriding the default Startup class when initializing the SwaggerUI.</td>
  </tr>
</table>
...
<table>
  ...
  <tr style={{verticalAlign: 'top'}}>
    <td><p style={{paddingLeft: '22px', textIndent: '-22px'}}>:new: ApiSettings<br/>:Behaviors:AllowWhitespaceOnlyValues</p></td>
    <td>false</td>
    <td>Overrides the default behavior changed in v7.3 of not allowing whitespace-only values in (non-key) properties.</td>
  </tr>

  <tr style={{verticalAlign: 'top'}}>
    <td>:new: ApiSettings:OneRosterVersionUrl</td>
    <td>`localhost:3000`</td>
    <td>Configures the root URL of the OneRoster service. The ODS / API uses this value to retrieve the OneRoster OpenAPI specification for integration into the ODS / API Swagger documentation.</td>
  </tr>

  <tr style={{verticalAlign: 'top'}}>
    <td>:new: FeatureManagement:OneRoster</td>
    <td>true</td>
    <td>Enables OneRoster integration in the Ed‑Fi ODS / API. When enabled, the ODS / API recognizes OneRoster as a supported feature and uses the configured `ApiSettings:OneRosterVersionUrl` to retrieve the OneRoster OpenAPI specification and include OneRoster endpoints in the ODS / API Swagger documentation. This feature also requires `Security:AccessTokenType` to be set to `jwt`</td>
  </tr>

  <tr style={{verticalAlign: 'top'}}>
    <td>:new: FeatureManagement:SerializedData</td>
    <td>true</td>
  ...
  <tr style={{verticalAlign: 'top'}}>
    <td>:new: FeatureManagement:ResourceLinks</td>
    <td>true</td>
  ...
  <tr style={{verticalAlign: 'top'}}>
    <td>:new: FeatureManagement:Extensions</td>
    <td>true</td>
  ...
  <tr style={{verticalAlign: 'top'}}>
    <td>:new: FeatureManagement:UniqueIdValidation</td>
    <td>false</td>
  ...
  <tr style={{verticalAlign: 'top'}}>
    <td>:new: FeatureManagement:TokenInfo</td>
    <td>true</td>
  ...
  <tr style={{verticalAlign: 'top'}}>
    <td>:new: StartupClassName</td>
    <td>CustomStartup</td>
    <td>Specifies the name of the Startup class the application should use. This allows overriding the default Startup class when initializing the API.</td>
  </tr>

  <tr style={{verticalAlign: 'top'}}>
    <td>:new: DomainsExcludedFromOpenApi</td>
    <td>Discipline, Staff</td>
    <td>Specifies which domains should be removed from the OpenAPI specification. Any endpoints or descriptors associated with the listed domains will be excluded.</td>
  </tr>

  <tr style={{verticalAlign: 'top'}}>
    <td>:new: Security:AccessTokenType</td>
    <td>guid</td>
    <td>Configures the type of access token issued by the API. By default, the APIuses legacy `guid`-based access tokens. Set this value to `jwt` enable issuance and validation of JWT-based OAuth 2.0 access tokens.</td>
  </tr>

  <tr style={{verticalAlign: 'top'}}>
    <td>:new: Security:Jwt</td>
    <td>Example: [see this document](../../how-to-guides/how-to-oneroster-with-the-ed-fi-ods-api.md#step-2-configure-jwtbased-security-in-the-ods--api)</td>
    <td>Required when `Security:AccessTokenType` is set to `jwt` for the API issue and validate JWT tokens. Configures JWT token issuance and validation, including issuer, audience, and public/private key material used for token signing.</td>
  </tr>
...
CLI Tool](https://learn.microsoft.com/en-us/aspnet/core/security/app-secrets?view=aspnetcore-10.0&tabs=windows#set-a-secret)
...
or [Manage User
Secrets](https://docs.microsoft.com/en-us/aspnet/core/security/app-secrets?view=aspnetcore-10.0&tabs=windows#json-structure-flattening-in-visual-studio)
in Visual Studio.
...
Core](https://learn.microsoft.com/en-us/aspnet/core/security/app-secrets?view=aspnetcore-10.0&tabs=windows)
for more details.
