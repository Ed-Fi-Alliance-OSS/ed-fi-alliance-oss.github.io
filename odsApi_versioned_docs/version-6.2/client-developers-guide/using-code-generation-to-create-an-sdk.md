...
ODS / API v6.2 metadata endpoints return documents adhering to the OpenApi
...
[https://api.ed-fi.org/v6.2/api/metadata/data/v3/swagger.json?version=2](https://api.ed-fi.org/v6.2/api/metadata/data/v3/swagger.json?version=3)
for OpenApi v2.0
[https://api.ed-fi.org/v6.2/api/metadata/data/v3/swagger.json?version=3](https://api.ed-fi.org/v6.2/api/metadata/data/v3/swagger.json?version=3)
or
[https://api.ed-fi.org/v6.2/api/metadata/data/v3/swagger.json](https://api.ed-fi.org/v6.2/api/metadata/data/v3/swagger.json)
for OpenApi v3.0
...
If you don't already have Java installed, navigate to [https://java.com/en/download/](https://java.com/en/download/) and download the latest installer. Run the installer to install the latest version of Java. In case you're wondering: the code generation leverages Java, but it does output C# code.
...
The SDK source files are generated using Swagger metadata via a few simple PowerShell commands. You can see the available metadata endpoints for SDK generation at [https://api.ed-fi.org/v6.2/api/metadata?sdk=true](https://api.ed-fi.org/v6.2/api/metadata?sdk=true). 
...
10.  Edit the **Program.cs** file and paste the following code. The client and key are using a publicly available sandbox environment with sample data hosted by the Ed-Fi Alliance.
    ...
    // Oauth configuration
    var oauthUrl = "https://api.ed-fi.org/v6.2/api/";
    ...
    var configuration = new Configuration() { AccessToken = tokenRetriever.ObtainNewBearerToken(), BasePath = "https://api.ed-fi.org/v6.2/api/data/v3" };
...
11.  **Add a .cs file** named **TokenRetriever.cs** and copy the following code to help with OAuth integration.
    ...
    ```
12.  Build the project and run it without debugging (**Ctrl**+**F5**)
