...
$token = Invoke-RestMethod -Method Post -Uri "https://api.ed-fi.org/v6.2/api/oauth/token" `
...
```bash
curl --user RvcohKz9zHI4:E1iEFusaNf81xzCxwHfbolkC https://api.ed-fi.org/v6.2/api/oauth/token --data 'grant_type=client_credentials'
```
...
```powershell
Invoke-RestMethod -Method Get -Uri https://api.ed-fi.org/v6.2/api/data/v3/ed-fi/schools `
...
```bash
curl https://api.ed-fi.org/v6.2/api/data/v3/ed-fi/schools -H "Authorization: Bearer R3PLAC3_W1TH_ACC3SS_TOK3N"
```
...
```powershell
Invoke-RestMethod -Method Get -Uri https://api.ed-fi.org/v6.2/api/data/v3/ed-fi/disciplineIncidents  `
  -Headers @{ "Authorization" = "Bearer $($token.access_token)" }
```
...
```bash
curl https://api.ed-fi.org/v6.2/api/data/v3/ed-fi/disciplineIncidents -H "Authorization: Bearer R3PLAC3_W1TH_ACC3SS_TOK3N"
```
