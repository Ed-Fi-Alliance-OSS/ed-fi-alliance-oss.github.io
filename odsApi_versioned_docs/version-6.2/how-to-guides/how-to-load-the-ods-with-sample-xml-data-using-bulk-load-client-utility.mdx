...
This article uses the core sample data released with [Ed-Fi Data Standard v4.0](https://edfi.atlassian.net/wiki/display/EFDS4x/). 
If you have an extension that extends core entities with additional required fields, the core sample data may fail to fulfill those additional 
requirements and fail to load.
...
*   [Step 1. Install the Ed-Fi Bulk Load Client](#step-1-install-the-ed-fi-bulk-load-client)
...
*   [Step 5. Run the Bootstrap Script to Load Descriptors, Standards and Education Organizations](#step-5-run-the-bootstrap-script-to-load-descriptors-standards-and-education-organizations)
*   [Step 6. Update Claim Set to Load Sample Data](#step-6-update-claim-set-to-load-sample-data)
*   [Step 7. Run the Bootstrap Script to Load Sample Data](#step-7-run-the-bootstrap-script-to-load-sample-data)
...
* Install the Ed-Fi Bulk Load Client from a PowerShell prompt using the
  following commands:
    ...
    c:\> dotnet tool install EdFi.Suite3.BulkLoadClient.Console --version 6.2.25 --tool-path {YourInstallFolder}
    You can invoke the tool using the following command: EdFi.BulkLoadClient.Console
    Tool 'edfi.suite3.bulkloadclient.console' (version '6.2.25') was successfully installed.
    ```
...
* **Download and Extract** `Ed-Fi-SampleDataLoad.zip` to a local folder. We
  recommend `C:\Ed-Fi-SampleDataLoad`. `Ed-Fi-SampleDataLoad.zip` contains all
  the scripts and directory structure used in this how-to article.
...
* **Navigate** to the `Ed-Fi-ODS-Implementation` directory
...
* **Execute CreateApiClientforEmptySandbox.sql** against the `EdFi_Admin`
  database. This will create claims needed to access the
  `EdFi_Ods_Sandbox_Empty`.
...
* Open the `LoadBootstrapData.ps1` file in a text editor and update the
...
* For Postgres Only, please update Key &amp Secret `"-k", "**E**mpty", "-s", "empty**S**ecret"`
...
* **Execute `EnableSandboxClaimset.sql` against the `EdFi_Admin` database. This
...
## Step 7. Run the Boostrap Script to Load Sample Data
...
this how-to article.
[Ed-Fi-SampleDataLoad.zip](https://edfi.atlassian.net/wiki/download/attachments/18219566/Ed-Fi-SampleDataLoad.zip?version=1&modificationDate=1710953737387&cacheVersion=1&api=v2)
The following GitHub links contain the sample XML files and the as-shipped
Ed-Fi Descriptor XML. [Sample XML](https://github.com/Ed-Fi-Alliance-OSS/Ed-Fi-Standard/tree/v4.0.0/Samples/Sample%20XML) 
[Descriptors](https://github.com/Ed-Fi-Alliance-OSS/Ed-Fi-Standard/tree/v4.0.0/Descriptors)
