...
| --- | --- | --- |
| `EdFi\_Ods\_Empty` | SQL Scripts | An empty database used for code generation when building the solution. |
| `EdFi\_Ods\_Minimal\_Template` | SQL Scripts | A template database used to create empty sandboxes. |
...
The Ed-Fi ODS / API comes with a set of configurable features that can be enabled or disabled using configuration settings in deployed API. See the [Configuration Details](../platform-dev-guide/configuration/configuration-details.md) section for more details on these settings.
...
| tokenInfo | true | Enables oauth [token\_info](https://tools.ietf.org/html/rfc7662#section-2) introspective endpoint which provides the additional security configuration information for the token. |
