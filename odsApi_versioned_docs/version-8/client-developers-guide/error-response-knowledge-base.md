...
The Ed-Fi API adheres to the REST principles when responding to HTTP requests,
including the use of standard status codes in HTTP responses. The API implements
the [Problem Details RFC 9457](https://www.rfc-editor.org/rfc/rfc9457.html).
This standardizes API error responses and enables machine-readable error details
within the HTTP response content.
...
- **`type`**: This field uniquely identifies the error type as specified in
  ...
  `urn:ed-fi:api:security:authorization:access-denied:action` and
  `urn:ed-fi:api:security:authorization:namespace:access-denied:namespace-mismatch`
  identify specific issue types within the context of an authorization error.
...
This document aims to catalog most of the Ed-Fi API error responses,
...
{
  "detail": "Identifying values for the Program resource cannot be changed. Delete and recreate the resource item instead.",
...
```text
POST http://localhost:8080/api/data/ed-fi/absenceEventCategoryDescriptors
Authorization: bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json
...
```text
POST http://localhost:8080/api/data/ed-fi/academicWeeks
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json; charset=utf-8
...
though a few may be Int64 (max value: 9,223,372,036,854,775,807).
...
```text
POST http://localhost:8080/api/data/ed-fi/communityOrganizations
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json; charset=utf-8
...
```text
POST http://localhost:8080/api/data/ed-fi/accountabilityRatings
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json; charset=utf-8
...
```text
POST http://localhost:8080/api/data/ed-fi/schools
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json; charset=utf-8
...
The Ed-Fi API expects UTF-8 character encoding. In the following example,
...
consume data from the Ed-Fi API.
...
```text
POST http://localhost:8080/api/data/ed-fi/accountabilityRating
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json; charset=utf-16
...
```text
POST http://localhost:8080/api/data/ed-fi/bellSchedules
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json; charset=utf-16
...
```text
POST http://localhost:8080/api/data/ed-fi/assessments
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json; charset=utf-16
...
Due to the Key Structure in the Ed-Fi API, a single resource may contain a
certain property, and it may reference another object that has that same
property. Often, the two values should be the same for consistency. We call
this "key unification". The example below has some interesting features:
...
```text
POST http://localhost:8080/api/data/ed-fi/studentSchoolAssociations
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json; charset=utf-16
...
```text
GET http://localhost:8080/api/data/ed-fi/studentSchoolAssociations?limit=-1
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
...
```text
PUT http://localhost:8080/api/data/ed-fi/studentAcademicRecords/d074a26cec7449299f701ba54e0f0257
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json; charset=utf-8
...
```text
POST http://localhost:8080/api/data/ed-fi/studentAcademicRecords
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json; charset=utf-8
...
```text
PUT http://localhost:8080/api/data/ed-fi/schools/310a0f89ada9413e9e9e46187bb1d10d
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
...

{
    ...
    "correlationId": "54e1e704-72a4-4bcd-b455-9c483352a84b",
    "errors": [
        "Resource class 'School' is not writable using API profile 'test-profile-resource-readonly'."
    ]
}
```
...
- Did the token time out? If so, re-authenticate with client credentials.
  - The token's lifetime is determined by the identity provider configuration
    (the Configuration Service's self-contained provider or Keycloak).
- Inspect the HTTP request to ensure it is well formatted. For more information
  on authentication and authorization, see [Authentication](./authentication.md)
...
```text
GET http://localhost:8080/api/data/ed-fi/students/82b0fb10c78e43a29ecd2d286ddc10c3
...
Most of these 403 message refer to the portion of the Ed-Fi API that uses
...
- Is the correct namespace being transmitted? If that is correct, then it could
  ...
  IT operations:
- Using a tool like the Configuration Service API, confirm that the key is
  configured for access to the intended namespace.
- Using the [Token Info](./authorization.md#token-info) endpoint, confirm that
  the key is configured for access to the intended namespace.

:::

### urn:ed-fi:api:security:authorization

#### Student / Contact / Staff Relationship Does Not Exist

This frequently trips up folks new to working with the Ed-Fi API: once you
create a student, contact, or staff, most\* client credentials cannot immediately
...
appropriate `studentSchoolAssociation`, `studentContactAssociation`, or
...
```text
## HTTP Request 1
POST http://localhost:8080/api/data/ed-fi/students
Authorization: bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json
...
## HTTP Request 2
GET http://localhost:8080/api/data/ed-fi/students/e31429919e6546e8905635d7858f2e80
...
location: http://localhost:8080/api/data/ed-fi/contacts/3f375bc2ac5c4a709a168616afb4772e
...
- Be sure to create an appropriate `studentSchoolAssociation`,
  `studentContactAssociation`, or `staffSchoolAssociation` record after creating
  the corresponding `Student`, `Contact`, or `Staff` object. However, it is
...
```text
POST http://localhost:8080/api/data/ed-fi/staffSectionAssociations
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json; charset=utf-8
...
- Be sure to create an appropriate association record only _after_ creating
  the corresponding `Student`, `Contact`, or `Staff` object.
...
This is similar to the student / contact / staff problem above. In this case, the
...
```text
POST http://localhost:8080/api/data/ed-fi/classPeriods
Authorization: Bearer fd964160527941a39875e821d2622088
Content-Type: application/json; charset=utf-8
{
  "schoolReference": {
...
```text
POST http://localhost:8080/api/data/ed-fi/studentEducationOrganizationAssociations
Authorization: Bearer fd964160527941a39875e821d2622088
Content-Type: application/json; charset=utf-8
...
- Interestingly, _this also occurs if the school does not exist_. This is a
  ...
  otherwise legitimate `schoolId`'s that they should not be able to access.
...
- Using the [Token Info](./authorization.md#token-info) endpoint, confirm that
  the key is configured for access to the given school or education organization.
...
Credentials to the Ed-Fi API are configured with a claim set that
...
```text
PUT http://localhost:8080/api/data/ed-fi/absenceEventCategoryDescriptors/3a62029e71374c7095d288a6cabae206
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json; charset=utf-16
...

{
  ...
  "errors": [
    "The API client's assigned claim set (currently 'SIS Vendor') must grant permission of the 'Update' action on one of the following resource claims: AbsenceEventCategoryDescriptor"
  ]
}
...
### urn:ed-fi:api:security:authorization:access-denied:action (missing resource claim)
...
```text
POST http://localhost:8080/api/data/ed-fi/academicWeek
Authorization: Bearer fd964160527941a39875e821d2622088
Content-Type: application/json; charset=utf-8
...
```text
PUT http://localhost:8080/api/data/ed-fi/academicWeek/not-valid-id
Authorization: Bearer fd964160527941a39875e821d2622088
Content-Type: application/json; charset=utf-8
...
```text
POST http://localhost:8080/api/data/ed-fi/studentAcademicRecords/8cdf37d9-f2cb-4104-8a5f-8cbaf96ca53f
Authorization: Bearer fd964160527941a39875e821d2622088
Content-Type: application/json; charset=utf-8
...
```text
PUT http://localhost:8080/api/data/ed-fi/studentAcademicRecords
Authorization: Bearer fd964160527941a39875e821d2622088
Content-Type: application/json; charset=utf-8
...
```text
DELETE http://localhost:8080/api/data/ed-fi/studentAcademicRecords
Authorization: Bearer fd964160527941a39875e821d2622088
...
- Append the `id` value at the end of the URL. In this case, the URL should
  be `http://localhost:8080/api/data/ed-fi/studentAcademicRecords/d074a26cec7449299f701ba54e0f0257`

:::

#### PATCH Requests

The Ed-Fi API doesn't support PATCH requests; to update data, use PUT requests
instead.
...
```text
PATCH http://localhost:8080/api/data/ed-fi/students/1765c75f180a4981b7a2a47974ba14d9
Authorization: Bearer fd964160527941a39875e821d2622088
Content-Type: application/json; charset=utf-8
{
...
```text
DELETE http://localhost:8080/api/data/ed-fi/students/3d3ad42503b443489471b74c05067628
Authorization: bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json
...
{
  "detail": "The requested action cannot be performed because this item is referenced by existing StudentSectionAttendanceEvent item(s).",
...
- Be sure to delete data following the required [Resource Dependency
  Order](./resource-dependency-order/readme.mdx) in reverse order (delete
  children first).
...
The Ed-Fi API enforces referential integrity: whenever a property is
...
School year is unusual in the Ed-Fi API in that it comes preconfigured. By
default, the Ed-Fi API recognizes school years 1991-2050. Each
implementation can change that list. There is a `schoolYearTypes` resource in
the API; however, not all credentials are able to access it.

HTTP Request

```text
POST http://localhost:8080/api/data/ed-fi/calendars
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json; charset=utf-8
...
```text
POST http://localhost:8080/api/data/ed-fi/chartOfAccounts
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json; charset=utf-8
...

{
  ...
  "validationErrors": {
    "$.fundDimensionReference": [
      "The referenced FundDimension item does not exist."
    ]
  },
  "errors": []
}
...
- Be sure to submit data following the required [Resource Dependency
  Order](./resource-dependency-order/readme.mdx).
...
### urn:ed-fi:api:identity-conflict

#### Identifying Values Are Not Unique

A PUT request can be used to update a natural key field on selected resources.
However, the API will error if a resource is being updated with a key that is
already in use by another resource. The Ed-Fi API enforces uniqueness of natural
...
```text
PUT http://localhost:8080/api/data/ed-fi/classPeriods/d327e6013bff4c7783ecdbbb420b23bb
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
Content-Type: application/json; charset=utf-8
...

{
  ...
  "status": 409,
  "errors": [
    "A natural key conflict occurred when attempting to update a resource ClassPeriod with a duplicate key. The duplicate keys and values are (ClassPeriodName = 01 - Traditional),(SchoolId = 255901044)"
  ],
...
GET http://localhost:8080/api/data/ed-fi/academicWeeks
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
...
PUT http://localhost:8080/api/data/ed-fi/academicWeeks/cc91fb951b0c4cb7ae0ed56ac0691fb6
Authorization: Bearer 6f5bb488a65948b5b847b561b23e
