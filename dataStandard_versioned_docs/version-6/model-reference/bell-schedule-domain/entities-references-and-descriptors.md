...
| EducationalEnvironment | Section | The setting in which a student receives education and related services. |     |     |     |     |
...
| GradeLevel | BellSchedule | The grade levels the particular BellSchedule applies to. | Orthodox | Yes | Yes |     |
...
| SectionCharacteristic | Section | Reflects important characteristics of the section, such as whether or not attendance is taken and the section is graded. |     |     |     |     |
| SectionType | Section | Specifies whether the section is for attendance only, credit only, or both. |     |     |     |     |
| StateAbbreviation | EducationOrganization | The abbreviation for the state (within the United States) or outlying area in which an address is located. |     |     |     |     |
| Term | Session | A descriptor value to indicate the term that the session is associated with. |     |     |     |     |
