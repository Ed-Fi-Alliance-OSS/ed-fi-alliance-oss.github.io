...
Data Standard v4 (current version 4.0-a) introduces important updates to 3
domains of the Ed-Fi data model:
...
applies only for the Staff entity. If MappingScope is omitted, then the mapping
...
<Tabs>
<TabItem value="assessments" label="On /assessments">
...
<TabItem value="studentAssessments" label="On /studentAssessments">
...
issues](https://tracker.ed-fi.org/issues/?jql=project+%3D+DATASTD+AND+issuetype+%3D+Story+AND+fixVersion+%3D+%22Data+Standard+v4.0-a%22+&src=confmacro)
