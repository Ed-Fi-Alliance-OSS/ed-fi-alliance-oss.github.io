...
documentation](/reference/ed-fi-api/platform-dev-guide/extensibility-customization/extending-the-ods-api-data-model) for more detail
...
See the [Ed-Fi ODS / API documentation](/reference/ed-fi-api) for more detail.
...
named `SqlDataDictionary.xlsx`. The Ed-Fi XSD Data Dictionary
provides documentation on core and extension XSD elements generated by MetaEd.
This Excel file is named `XmlDataDictionary.xlsx`.
...
* `Ed-Fi-Data-Handbook-Index.html` - An interactive and searchable version of the
    data handbook.
...
New in MetaEd 4.7, released in May 2028.

:::

This Excel spreadsheet contains two tabs: Resources and Properties. These describe components of the data model as they will be translated into the Open API ("Swagger") specification in the Ed-Fi API. In other words, this spreadsheet matches what API clients actually see in production, whereas the Data Handbook above describes only the _conceptual data model_.

The **Resources** tab contains:

| Column               | Example                                          |
| -------------------- | ------------------------------------------------ |
| Project              | "ed-fi" or the extension name                    |
| Version              | 5.2.0                                            |
...
The file will be named `Ed-Fi-API-Catalog.xlsx` and will appear in the `Ed-Fi-API-Catalog` folder.
...
Example](/reference/ed-fi-api/how-to-guides/how-to-extend-the-ed-fi-ods-api-student-transcript-example) for
