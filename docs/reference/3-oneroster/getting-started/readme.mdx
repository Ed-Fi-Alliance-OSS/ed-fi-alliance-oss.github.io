...
| [Deploy on IIS (Windows)](./deploy-iis.md)          | You are running the Node service on Windows Server and want to use IIS as a reverse proxy in front of it.             | Production-supported      |

For production Docker deployments, use the official
[`edfialliance/one-roster-api`](https://hub.docker.com/r/edfialliance/one-roster-api)
image — the in-repo Docker Compose stack is for demo only.

:::info Before you begin: ODS / API configuration
...
API](/reference/ed-fi-api/how-to-guides/how-to-oneroster-with-the-ed-fi-ods-api).
...
- SQL Server 2016 or later (required for the JSON functions used by the refresh
  procedures)
