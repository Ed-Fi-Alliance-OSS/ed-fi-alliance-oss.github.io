...
.DESCRIPTION
  ...
  create() flow: validate URLs, auto-detect version/tenant-mode, register OAuth
  credentials at the Admin API /connect/register, AES-encrypt the secret into
  configPrivate, and sync ODS instances / Education Organizations.

  AUTHENTICATION
  The /sb-environments endpoints require a passport session cookie (default name
  "connect.sid"). Sign in to the Admin App UI as the global admin, copy the
  "connect.sid" cookie value (DevTools -> Application/Storage -> Cookies ->
  https://localhost), and pass it as -SessionCookie. (-BearerToken is also
  accepted for an Auth0-backed, non-Keycloak deployment.)
#>
param(
...
# PowerShell 7.4+ refuses HTTPS->HTTP redirects (the local nginx 302s can resolve
# to an insecure hop) unless -AllowInsecureRedirect is passed. The switch does
# not exist before 7.4, so probe for it.
...
# ---- Step 4: create the environment (POST /sb-environments) -------------------
...
4. **`POST /sb-environments`** — creates the environment. The API validates the
   URLs, detects version/tenant-mode, registers and encrypts the Admin API
   credentials, and syncs the ODS instances / Ed-Orgs.
