...
5. On the next page you will be prompted to enter in an Application Name, select an Education Organization, select a Vendor, and select a Claimset.
   ...
   The **Ed-Org** will be the education organization with which you would like the API credentials to be associated. This will generally mean that any data in your environment that is related to that Ed-Org will be accessible via the application credentials you create. If this dropdown is empty because your ODS already contained education organizations before the Admin App was installed, see [Education Organization Sync](./edorg-synchronization/readme.md) for the one-time import that populates it.
