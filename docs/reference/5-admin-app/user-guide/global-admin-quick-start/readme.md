...
| SQL Server reachable | Both SQL Server databases the scripts touch — the Admin App application database and the ODS/API's `EdFi_Security` — may be local or remote, including a managed Azure SQL Database, which requires SQL authentication; see [Managed Azure SQL Database](quick-start-appendix#managed-azure-sql-database) |
