...
to enhance data integration and management capabilities for educational
...
current ODS/API Platform with the DMS by the 2028–2029 school year. In the
meantime, the Alliance remains committed to supporting the ODS/API Platform
through the 2027–2028 school year.
...
application, tentatively called the Ed-Fi Configuration Service. The plan is to
implement the Admin API specification (version 2), so that applications and
