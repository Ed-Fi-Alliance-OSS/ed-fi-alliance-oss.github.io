...
Repository: [Ed-Fi-ODS-Docker](https://github.com/Ed-Fi-Alliance-OSS/Ed-Fi-ODS-Docker)

* Use the [Releases
  page](https://github.com/Ed-Fi-Alliance-OSS/Ed-Fi-ODS-Docker/releases) to find
  the most recently published versions. Images:
  [https://hub.docker.com/u/edfialliance](https://hub.docker.com/u/edfialliance)
...
Architecture](./docker-deployment/ed-fi-docker-compose-architecture.md) for more
...
1. Run installations from the official binary distributions and documentation:
     ...
     :checkmark: PRO: can get the minimal or populated template
     :x: CON: can only run on localhost - requires a migration process
...
2. Use experimental
    ...
    Admin API and to allow remote deployments.
     ...
     :checkmark: PRO: should be able to install into any destination MSSQL database you can
     reach
     :x: CON: no sample data (minimal or populated template)
...
:warning: Please change these values to more secure settings before using them
...
In this step, you need to choose between deploying a **Sandbox,**
**SingleTenant, SingleTenant with ODS Context routes, MultiTenant, or
MultiTenant with ODS Context routes** environment, as described in [Docker
Compose Architecture:
ConfigurationModes](./ed-fi-docker-compose-architecture.md).
The source code repository contains a Docker compose orchestration file for each
scenario. The sample commands below show the use of the published images from
Docker Hub. These commands can be run from any shell terminal that supports
Docker commands (e.g., PowerShell in Windows).
...
* **odsContextRouteTemplate** - Defines the route parameter template used to
    ...
    {/* Configure it following [route
    constraints](<https://techdocs.ed-fi.org/docker> run --rm -v
    c:%5CEd-Fi-ODS-Docker%5CCompose-Generator%5CAlpine%5Ctemplates%5Cpgsql%5Cparameters-year-specific.yml:/parameters
    -v C:%5CEd-Fi-ODS-Docker%5CCompose%5Cpgsql%5CSingleTenant-OdsContext:/output
    edfialliance/ods-compose-generator). */}

* **contextKey** \- Key used to configure dbo.OdsInstanceContext table in
    EdFi\_Admin database*. Should match URI in odsContextRouteTemplate*
...
Note that the parameters file path must be fully qualified (Docker
volumes don't support relative paths).
For example:
...
![Sandbox Postgres](https://edfidocs.blob.core.windows.net/$web/img/reference/docker/image-2023-9-29_18-12-53.png)
...
![Single Tenant Postgres](https://edfidocs.blob.core.windows.net/$web/img/reference/docker/image-2023-9-29_17-53-7.png)
...
![](https://edfidocs.blob.core.windows.net/$web/img/reference/docker/image-2023-9-30_0-4-16.png)
...
![](https://edfidocs.blob.core.windows.net/$web/img/reference/docker/image-2023-9-30_0-7-1.png)
...
![](https://edfidocs.blob.core.windows.net/$web/img/reference/docker/image-2023-9-30_0-24-26.png)
...
could set the LOGS\_FOLDER variable to c:/tmp/logs for Windows hosts or to
