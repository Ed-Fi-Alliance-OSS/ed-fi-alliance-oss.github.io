...
* [Google Classroom](https://github.com/Ed-Fi-Alliance-OSS/LMS-Toolkit/blob/main/docs/demonstration/google-e2e.ipynb) **note the requirement for a `service-account.json` file**.
...
1. Install the `lmsx`  schema tables through one of two options:
   ...
   1. Create a new file called `lmsx.ps1` in the `Ed-Fi-ODS-Implementation/Plugin` directory, using the contents shown below
      :::warning
   ...
   2. In your `Ed-Fi-ODS-Implementation/Application/EdFi.Ods.WebApi` directory, run the following commands:
