...
:::info[Updated 2026-06-17]

This post has been updated to reflect the latest implementation. The most significant change since its original publication is that **change tracking is now implemented in the schema** (see _Change Tracking and Change Queries_ below).
...
Three more support schema governance rather than carrying data. **`dms.ResourceKey`** maps each `(ProjectName, ResourceName)` pair to the compact `smallint ResourceKeyId` used on `dms.Document` and `dms.ReferentialIdentity`. **`dms.EffectiveSchema`**/**`dms.SchemaComponent`** record which Ed‑Fi API schema and extension schemas the deployed database was built from, so API can confirm the database matches the expected API schema.
